...
class UsersController < ApplicationController
  def index
    ...
    @requests_sent_ids = current_user.sent_friendships.where(approved: false).pluck(:receiver_id)
    @requests_received_ids = current_user.received_friendships.where(approved: false).pluck(:sender_id)
